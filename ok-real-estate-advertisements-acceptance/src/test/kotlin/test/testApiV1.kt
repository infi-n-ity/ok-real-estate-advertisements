...
fun FunSpec.testApiV1(client: Client) {
    ...
    test("Update Ad ok") {
        ...
        client.readAd(created.id) {
            ...
            //it.ad?.realEstateType shouldBe RealEstateType.HOUSE
    ...
    test("Search Ad ok") {
        ...
        withClue("Search Selling") {
            ...
            //results shouldExist { it.realEstateType == RealEstateType.FLAT }
            //results shouldExist { it.realEstateType == RealEstateType.HOUSE }
        }

        withClue("Search Bolt") {
            client.searchAd(search = AdSearchFilter(searchString = "Astana"))
            // TODO раскомментировать, когда будет реальный реп
//             .shouldExistInOrder({ it.realEstateArea == "Astana city" })
        }
    }
}